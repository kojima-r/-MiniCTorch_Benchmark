...
class VAE(torch.nn.Module):
  ...
  def __init__( self, n_in, n_mid, n_out, n_z ):
    ...
    print("create ae class")
    self.net  = Net( n_in, n_mid, n_out, n_z )
    self.loss = Loss()

  def forward( self, x ):
    y = self.net(x)
    output = self.loss( y, x )
    return output
...
def experiment_pytorch(x_train, batch_size=32, epochs=200,n_mid=32,n_z=2):
    ...
    ae = VAE( n_in, n_mid, n_out, n_z )
    ae.train()

    lr = 0.001
    opt = torch.optim.SGD( ae.parameters(), lr)
    #opt = torch.optim.Adam( ae.parameters(), lr )
    ...
    for i in range(epochs):
        ...
        ae.eval()
        loss = ae( x2 )
        #print("loss", loss, ae.loss.loss1, ae.loss.loss2 )
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dim_z', type=int, default=2,
                        help='')
    parser.add_argument('--output_dir', type=str, default="./example_ae",
                        help='example_dir')
